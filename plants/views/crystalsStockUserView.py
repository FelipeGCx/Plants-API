...
class CrystalStockUserView(APIView):
    ...
    def get(self, request, user=None, format=None):
        ...
        if named is not None:
            crystalisStock = list(sorted(crystalisStock, key=lambda x: named.lower() in x.name.lower(), reverse=True))


        vibration_first = self.request.query_params.get('vibrationFirst', None)
        if vibration_first is not None:
            vibration_second = self.request.query_params.get(
                'vibrationSecond', None)
            if vibration_second is not None:
                crystalisStock = list(filter(lambda x: x.vibration >= int(
                    vibration_first) and x.vibration <= int(vibration_second), crystalisStock))
