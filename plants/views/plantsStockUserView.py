...
class PlantsStockUserView(APIView):
    ...
    def get(self, request, user=None, format=None):
        ...
        if name is not None:
            ...
            plantisStock = list(filter(lambda x: name.lower() in x.name.lower(
            ) or name.lower() in x.other_names.lower(), plantisStock))

        species = self.request.query_params.get('species', None)
        if species is not None:
            # plantisStock = plantisStock.filter(species = species)
            plantisStock = list(
                filter(lambda x: x.species.lower() == species.lower(), plantisStock))

        group = self.request.query_params.get('group', None)
        if group is not None:
            # plantisStock = plantisStock.filter(group = group)
            plantisStock = list(
                filter(lambda x: x.group.lower() == group.lower(), plantisStock))
        ...
        if irrigation is not None:
            ...
            for i in irrigation:
                ...
                plantisStock = list(
                    filter(lambda x: i.lower() in x.irrigation.lower(), plantisStock))
        ...
        if flowering is not None:
            ...
            plantisStock = list(
                filter(lambda x: x.flowering == flowering, plantisStock))
        ...
        if inside is not None:
            ...
            plantisStock = list(
                filter(lambda x: x.inside == inside, plantisStock))
        ...
        if price_first is not None:
            ...
            if price_second is not None:
                ...
                plantisStock = list(filter(lambda x: x.price >= int(
                    price_first) and x.price <= int(price_second), plantisStock))
        ...
        for item in plantisStock:
            if item.state:
                fav = PlantFavorite.objects.filter(
                    id_user=user, id_plant=item.id)
                val = {
                    "id": item.id,
                    "name": item.name,
        ...
        order = self.request.query_params.get('order', None)
        if order is not None:
            data = sorted(data, key=lambda x: x[str(order).replace(
                "-", "")], reverse=True if "-" in order else False)
